        ...
        <div class="row">
            <div class="col-xl-4">
                <div class="row gy-2 pb-2 gx-2">
                    <div class="col-sm-6 col-xl-12">
                        <a href="{{ route('admin.agency.report.transaction') }}?search={{ $user->username }}">
            ...
            <div class="col-xl-8">
                <div class="card">
                    <div class="card p-2">
                        <ul class="d-flex flex-wrap gap-1">
                            <li class="flex-grow-1 flex-shrink-0">
                                <a class="d-block btn bg--primary" href="{{route('admin.agencies.login',$user->id)}}"
                                    target="_blank">
                                    <i class="las la-sign-in-alt"></i> @lang('Login as Agency')
                                </a>
                            </li>
                            <li class="flex-grow-1 flex-shrink-0">
                                <a class="d-block btn bg--primary"
                                    href="{{ route('admin.agencies.notification.log',$user->id) }}">
                            ...
                            <li class="flex-grow-1 flex-shrink-0">
                                ...
                                <a class="d-block btn bg--primary" class="userStatus" data-bs-toggle="modal"
                                    ...
                                    <i class="las la-ban"></i> @lang('Ban Agency')
                                </a>
                                @else
                                <a class="userStatus bg--primary" data-bs-toggle="modal"
                                    data-bs-target="#userStatusModal" href="javascript:void(0)">
                                    <i class="las la-undo"></i> @lang('Unban Agency')
                                </a>
                                @endif
                            </li>
                            @if($user->kyc_data)
                            ...
                            <li class="flex-grow-1 flex-shrink-0">
                                <a class="d-block btn bg--primary"
                                    href="{{route('admin.agencies.notification.single', $user->id)}}">
                    ...
                    <div class="card-body">
                        <form action="{{route('admin.agencies.update',[$user->id])}}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="row">
...
<div id="addSubModal" class="modal fade" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="{{route('admin.agencies.add.sub.balance',$user->id)}}" method="POST">
                @csrf
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">
                    @if($user->status == 1)
                    <span>@lang('Ban Agency')</span>
                    @else
                    <span>@lang('Unban Agency')</span>
                    @endif
                </h5>
                <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                    <i class="las la-times"></i>
            ...
            <form action="{{route('admin.agencies.status',$user->id)}}" method="POST">
                @csrf
                <div class="modal-body">
                    @if($user->status == 1)
